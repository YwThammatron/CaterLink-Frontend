                                                                            ...
                                                                            {data.package_details.map((subdata,k) => {
                                                                                return (
                                                                                    <div key={"packageDetail"+k} className="relative flex">
                                                                                        ...
                                                                                        <p className={`relative ml-auto text-[14px] font-[400] ${data.discount == null || CompareDate(data.end_discount_date,FormatDate(new Date())) || CompareDate(FormatDate(new Date()),data.start_discount_date) ? 'text-[#475467]' : 'text-[#D92D20]'}`}> {data.discount == null || CompareDate(data.end_discount_date,FormatDate(new Date())) || CompareDate(FormatDate(new Date()),data.start_discount_date) ? subdata.price : `${subdata.price} -> ${(Math.round(subdata.price*((100-data.discount)/100)))}`} บาท/ที่</p>
                                                                                    </div>
                                                                                )
                                                                            })}
                                                                        </AccordionContent>
