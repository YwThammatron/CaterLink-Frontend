...
function DeletePackage() {
    ...
    const handleChangeDelete = async () => {
        //when click delete
        let isconfirm = window.confirm("System : Are you sure to delete this package? this process can not be reversed.")
        if(Packageid != "" && isconfirm){
            try{
                const response = await axios.delete(baseUrl + "/api/packages/" + Packageid)
                console.log(response.data)
                window.alert(`System : Package id ${Packageid} deleted.`)
    ...
    const getPackages = async () => {
        const response = await axios.get(baseUrl + "/api/packages/category/" + Packagectgid + "/" + Restid)
                       ...
                       {/* Input Field */}
                        <form className="grid w-[512px]">
                            ...
                            <div className="relative">
                                <select id="packagectgid" value={Packagectgid} onChange={(event) => setPackagectgid(event.target.value)} className="appearance-none w-[512px] h-[40px] text-[14px] pl-[12px] pr-[44px] pt-[10px] pb-[10px] border-[1px] border-[#D0D5DD] rounded-md">
                            ...
                            <div className="relative">
                                <select id="packageid" value={Packageid} onChange={(event) => setPackageid(event.target.value)} className="appearance-none w-[512px] h-[40px] text-[14px] pl-[12px] pr-[44px] pt-[10px] pb-[10px] border-[1px] border-[#D0D5DD] rounded-md">
